...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  filterTable () {
    ...
    if (queryParam) {
      ...
      queryParam = DOMPurify.sanitize(queryParam); // Sanitize input
      this.ngZone.runOutsideAngular(() => {
        this.io.socket().emit('verifyLocalXssChallenge', queryParam)
      })
      this.dataSource.filter = queryParam.toLowerCase()
      this.searchValue = this.sanitizer.sanitize(SecurityContext.HTML, queryParam) // Use Angular's built-in sanitizer to encode input
