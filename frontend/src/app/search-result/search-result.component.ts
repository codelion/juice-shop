...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  filterTable () {
    ...
    if (queryParam) {
      ...
      queryParam = DOMPurify.sanitize(queryParam); // Sanitizing the user input

      this.ngZone.runOutsideAngular(() => {
        this.io.socket().emit('verifyLocalXssChallenge', queryParam)
      })
      this.dataSource.filter = queryParam.toLowerCase()
      this.searchValue = queryParam; // Removed bypassSecurityTrustHtml to prevent trust of unsanitized input
