  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token) {
      res.status(200).json({
        ...
        body: DOMPurify.sanitize(`Hi, I can't recognize you. Sign in to talk to ${config.get<string>('application.chatBot.name')}`)
      })
      return
    }
    ...
    if (!username) {
      res.status(200).json({
        action: 'namequery',
        body: DOMPurify.sanitize('I\'m sorry I didn\'t get your name. What shall I call you?')
      })
      return
    }
    ...
    try {
      ...
      res.status(200).json({
        status: bot.training.state,
        body: DOMPurify.sanitize(bot.training.state ? bot.greet(`${user.id}`) : `${config.get<string>('application.chatBot.name')} isn't ready at the moment, please wait while I set things up`)
      })
    } catch (err) {
      next(new Error('Blocked illegal activity by ' + req.socket.remoteAddress))
    }
